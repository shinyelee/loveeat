	...
	<section id="ViewContainer">
		<div id="RecipeContainer">
			...
			<div id="recipe_thumbnail">
				<h2><span class="blueTitle">#${tag}</span>에 해당하는 레시피 보기</h2>
			...
			<div class="paging">
				<c:if test="${page.prev}">
				 <span>[ <a href="/recipe/search?tag=${tag}&num=${page.startPageNum - 1}">이전</a> ]</span>
				</c:if>
				...
				<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
				 <span>
				  ...
				  <c:if test="${select != num}">
				   <a href="/recipe/search?tag=${tag}&num=${num}">${num}</a>
				  </c:if>    
